...
async function load() {
  ...
  error.value = null
  try {
    const [deviceData, interfaceData, addressData, segmentData, connectionData] = await Promise.all([
      ...
      networkApi.connections(deviceId.value)
    ])
  ...
  <v-container fluid class="pa-4 pa-md-6">
    ...
    <template v-if="device">
            ...
            <v-expansion-panel v-for="item in interfaces" :key="item.id">
              <v-expansion-panel-title>
                <div class="d-flex flex-wrap align-center ga-2 w-100">
                  ...
                  <v-chip v-if="item.speed_mbps" size="x-small" variant="outlined">{{ item.speed_mbps }} Mbit/s</v-chip>
      ...
      <v-card v-if="device.role === 'switch' && physicalInterfaces.length" title="Switch-Frontansicht" prepend-icon="mdi-switch" class="mb-5">
        <v-card-text>
          <div class="switch-front" role="list" aria-label="Dokumentierte Switch-Ports">
            <v-tooltip v-for="port in physicalInterfaces" :key="port.id" :text="`${port.name}: ${port.speed_mbps ?? 'unbekannte'} Mbit/s`">
              <template #activator="{ props: tooltipProps }">
                <button v-bind="tooltipProps" class="switch-port" :class="{ disabled: !port.enabled }" type="button">
                  {{ port.name }}
                </button>
              </template>
            </v-tooltip>
    ...
    <v-dialog v-model="deviceDialog" max-width="680">
      <v-card title="Netzwerkprofil bearbeiten" prepend-icon="mdi-server-network">
        <v-card-text>
          ...
          <v-text-field v-model="deviceForm.hostname" label="Hostname" class="mb-3" />
...
.switch-front { display: grid; grid-template-columns: repeat(auto-fill, minmax(72px, 1fr)); gap: 10px; padding: 16px; border: 4px solid rgba(var(--v-theme-on-surface), .35); border-radius: 12px; background: rgba(var(--v-theme-on-surface), .05); }
.switch-port { min-height: 48px; border: 2px solid rgb(var(--v-theme-primary)); border-radius: 5px; background: rgb(var(--v-theme-surface)); color: rgb(var(--v-theme-on-surface)); cursor: default; font-size: .75rem; }
