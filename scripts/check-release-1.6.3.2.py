"""Verify release and electrical-integrity contracts for DocOfHome 1.6.3.2."""
...
def main() -> int:
    if read("VERSION").strip() != "1.6.3.2":
        raise AssertionError("VERSION ist nicht 1.6.3.2")
    require("backend/pyproject.toml", ('version = "1.6.3.2"',))

    package = json.loads(read("frontend/package.json"))
    lock = json.loads(read("frontend/package-lock.json"))
    if package["version"] != "1.6.3.2":
        raise AssertionError("frontend/package.json ist nicht 1.6.3.2")
    if lock["version"] != "1.6.3.2" or lock["packages"][""]["version"] != "1.6.3.2":
        raise AssertionError("Eigene package-lock-Metadaten sind nicht 1.6.3.2")

    source_info = json.loads(read("SOURCE_INFO.json"))
    expected = {
        "version": "1.6.3.2",
        "base_version": "1.6.3",
        "release_notes": "RELEASE_NOTES_1.6.3.2.md",
        "alembic_head": "0044",
    }
    ...
    require("README.md", ("DocOfHome 1.6.3.2", "Update von 1.6.3.1 auf 1.6.3.2"))
    require("RELEASE_NOTES_1.6.3.2.md", ("vollständig überdeckten", "direkten", "0044"))
    print("Release 1.6.3.2: Versions-, Elektro-, Migrations- und Paketverträge vorhanden.")
    return 0
