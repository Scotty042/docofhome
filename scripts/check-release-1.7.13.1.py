...
required = [
    ("VERSION", "1.7.13.1"),
    ("SOURCE_INFO.json", '"base_version": "1.7.13"'),
    ...
    ("RELEASE_NOTES_1.7.13.1.md", "# DocOfHome 1.7.13.1"),
]
for relative, marker in required:
    if marker not in (root / relative).read_text(encoding="utf-8"):
        raise SystemExit(f"1.7.13.1-Vertrag fehlt in {relative}: {marker}")
print("DocOfHome-Releasevertrag 1.7.13.1 geprüft.")
