...
DocOfHome 1.6.0 ist ein lokaler digitaler Zwilling für die technische
...
Start werden die Alembic-Migrationen bis Head `0037` ausgeführt. Der persistente
Ordner `./data` enthält Datenbank, Uploads, Backups und Laufzeitdaten und gehört
nicht in ein Quellcode-Release.

## Update von 1.5.0 auf 1.6.0

1. In DocOfHome ein Backup erstellen und den persistenten `data`-Ordner extern
   sichern.
2. Container stoppen: `docker compose down`.
3. 1.6.0 in einen neuen, sauberen Ordner entpacken.
4. Lokale `.env`- und Compose-Anpassungen übernehmen.
5. `docker compose build --no-cache` und `docker compose up -d` ausführen.
6. Migration `0037`, Healthcheck, Logs, Backups, Assistent, Schrankansicht und
   Smart-Meter-Messpunkte prüfen.

Migration `0037` ergänzt optionale Sicherungsautomat- und
Stromstoßschalter-Stammdaten sowie die neuen Smart-Meter-Messpunkt-Tabellen. Bestehende Daten bleiben erhalten.
