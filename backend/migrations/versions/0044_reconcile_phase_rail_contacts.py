"""Reconcile phase-rail contacts created after the 1.6.3 migration.
...
def upgrade() -> None:
    # 0043 repaired only rails that already existed during that upgrade. This
    # follow-up migration repeats the idempotent reconciliation for rails created
    # afterwards on installations affected by the runtime synchronization bug.
    _repair_derived_connections(op.get_bind())


def downgrade() -> None:
    # Derived physical contacts are valid domain data and remain in place.
    pass
